...
class EdgeCloudEcosystem extends AggregateProgram
  ...
  val TIME_STOP_ALL_REQUESTS = 400
  ...
  lazy val gcTime = Prop[Int]("gcTime", -1)

  lazy val role = Prop[Int]("role", 0)
  ...
  lazy val isLeader = Prop[Boolean]("leader", false).onChange{ case (prev,curr) => {
    if(!prev.getOrElse(false) && curr){ // i.e., become leader
      ...
      areaLoad.write(0)
      areaUtilization.write(0)
      areaStats.write(null)
      requestHistory.write(null)
      statActiveConsumers.write(0)
      statActiveWorkers.write(0)
    }}}
  lazy val leaderFailureProbsAlongTime = Prop[List[Double]]("leaderFailureProbsAlongTime", List.empty)
  lazy val supportedServices = Prop[Set[String]]("supportedServices", Set.empty) // services supported by workers
  lazy val favouriteServices = Prop[Set[String]]("favouriteServices", Set.empty)
  lazy val ongoingRequests = Prop[Set[Request]]("ongoingRequests", Set.empty) // services currently offered by workers to clients
    .onChange { case(_,r) =>
    localWorkerData.write(localWorkerData.get.copy(ongoingRequests = r, time = timestamp()))
  }
  lazy val completedRequestsAtWorker = Prop[Set[Request]]("completedRequestsAtWorker", Set.empty)
  lazy val workerFailureProbsAlongTime = Prop[List[Double]]("workerFailureProbsAlongTime", List.empty)
  lazy val localResources = Prop[LocalResources]("workerResources", LocalResources())
  lazy val localWorkerData = Prop[WorkerData]("workerProfile", null) // worker profile
  lazy val requests = Prop[Set[Request]]("localRequests", Set.empty)
    .onChange{ case (_,r) => {
      localConsumerData.write(ConsumerData(mid, getPosition(), r, timestamp()))
      activeRequestCount.write(r.size)
    } }
  lazy val requestsCompleted = Prop[Int](name="requestsCompleted", 0)
  lazy val localConsumerData = Prop[ConsumerData]("consumerProfile", ConsumerData(mid,getPosition(),Set.empty,0)) // worker profile
  lazy val moveTo = Prop[LatLongPosition]("move_to", null)
  lazy val allocations = Prop[Allocation]("allocations", Allocation())
  lazy val areaStats = Prop[AreaStats]("areaStats", null)
  lazy val round = Prop[Int]("round", 0)
  lazy val requestHistory = Prop[RequestHistory]("requestHistory", RequestHistory())
  lazy val activeRequestCount = Prop[Int]("activeRequestCount", 0)
  lazy val localEvents = Prop[Set[Event]]("localEvents", Set.empty)
  lazy val areaLoad = Prop[Double]("areaLoad", 0.0)
  lazy val areaUtilization = Prop[Double]("areaUtilization", 0.0)
  lazy val statReceivedRequests = Prop[Int]("requestsReceived", 0)
  lazy val statHandledRequests = Prop[Int]("requestsHandled", 0)
  lazy val statUnhandledRequests = Prop[Int]("requestsUnhandled", 0)
  lazy val statCompletedRequests = Prop[Int]("requestsCompletedAtLeader", 0)
  lazy val statDeclinedRequests = Prop[Int]("requestsDeclined", 0)
  lazy val statGarbageRequests = Prop[Int]("requestsGarbage", 0)
  lazy val statAllocatedRequests = Prop[Int]("statAllocatedRequests", 0)
  lazy val statFailedRequests = Prop[Int]("statFailedRequests", 0)
  lazy val statActiveConsumers = Prop[Int]("statActiveConsumers", 0)
  lazy val statActiveWorkers = Prop[Int]("statActiveWorkers", 0)
  lazy val relayCommunicationLoad = Prop[Long]("relayCommunicationLoad", 0)
  lazy val totalWaitingTime = Prop[Double]("totalWaitingTime", Double.NaN)
    .onChange{ case (_,d) => meanWaitingTime.write(d / requestsCompleted.get) }
  lazy val meanWaitingTime = Prop[Double]("meanWaitingTime", 0)
  lazy val smartAllocation = Prop.prop[Boolean,Double]("allocationStrategy", false, _>0, if(_) 1.0 else 0.0)
  lazy val numLeaderFailures = Prop[Int]("numLeaderFailures", 0)
  lazy val numWorkerFailures = Prop[Int]("numWorkerFailures", 0)
  lazy val liveWorkerPool = Prop[Map[ID,Long]]("liveWorkerPool", Map.empty)
  ...
  def roundInitialization: Unit = {
    ...
    branchOn[Unit](isConsumer && requests.isEmpty && currentTime().getSecond<TIME_STOP_ALL_REQUESTS){
  ...
  override def main(): Any = {
    ...
    val leaders = branch(FOG){ S(grain, metric = nbrRange _) }{ false }
...
//      node.put("debug_nbrs", foldhood(Set.empty[ID])(_++_)(Set(nbr(mid))))
      ...
      if(isConsumer || isWorker) {
        ...
        node.put("area", df.myLeader.getOrElse(-1))
      }
...
*/
  ...
  def dataDownstreaming(g: GlobalStructure, cd: ControlData): DownstreamData = {
    val myLeader = broadcast(mid, along = g.potential, metric = nbrRange _)
    val allocation = broadcast(cd.allocation, along = g.potential, metric = nbrRange _)
    val events = broadcast(localEvents, along = g.potential, metric = nbrRange _)
  ...
  def broadcast[T](value: T, along: Double, metric: Metric = nbrRange _) =
  ...
  case class Prop[T](name: String, default: T){
    var onChangeFunction: Option[(Option[T],T)=>Unit] = None

    def molecule = new SimpleMolecule(name)
    def get: T = _get
    def write(v: T) = {
      onChangeFunction.foreach(_(Try(get).toOption,v))
      _put(v)
    }
    def delete = {
      val n = node.asInstanceOf[SimpleNodeManager[_]].node
    ...
    protected def _get: T = if(node.has(name)) node.get[T](name) else default
  ...
  object Prop {
    ...
    def prop[V,Underlying](name: String, default: V, from:Underlying=>V, to:V=>Underlying): Prop[V] = new Prop[V](name, default){
